...
def Log_Datapath(dut,logger):
    ...
    dut._log.info("SrcB:%s", ToHex(dut.my_datapath.SrcB_pre.value))
...
def Log_Controller(dut,logger):
    ...
    dut._log.info("WdSrc:%s", ToHex(dut.my_controller.WdSrc.value))
    dut._log.info("RdSrc:%s", ToHex(dut.my_controller.RdSrc.value))
